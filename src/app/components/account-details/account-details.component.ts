...
export class AccountDetailsComponent implements OnInit, OnDestroy {
  ...
  addressBookMatch = '';
  amounts = [
    { name: 'OSLO', shortName: 'XNO', value: 'mnano' },
  ...
  remoteVisible = false;
  blockTypes: string[] = ['Send Oslo', 'Change Representative'];
  ...
  async generateSend() {
    ...
    if (!this.validateAmount()) return this.notifications.sendWarning(`Invalid OSLO Amount`);
    ...
    if (from.balanceBN.minus(rawAmount).lessThan(0)) return this.notifications.sendError(`From account does not have enough OSLO`);
