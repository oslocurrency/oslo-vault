...
export class WalletService {
        ...
        if (shouldNotify || transaction.block.subtype === 'change') {
          ...
          if (transaction.block.subtype === 'send') {
            ...
            if (this.addressBook.getTransactionTrackingById(addressLink)) {
              this.notifications.sendInfo(`Tracked address ${accountHrefLink} can now receive ${trackedAmount} OSLO`, { length: 10000 });
              console.log(`Tracked incoming block to: ${address} - Ӿ${trackedAmount}`);
            }
            // Outgoing transaction
            if (this.addressBook.getTransactionTrackingById(address)) {
              this.notifications.sendInfo(`Tracked address ${accountHref} sent ${trackedAmount} OSLO`, { length: 10000 });
            ...
            // Receive transaction
            this.notifications.sendInfo(`Tracked address ${accountHref} received incoming ${trackedAmount} OSLO`, { length: 10000 });
  ...
  async processPendingBlocks() {
    ...
    if (newHash) {
      ...
      this.notifications.sendSuccess(`Successfully received ${receiveAmount.isZero() ? '' : receiveAmount.toFixed(6)} OSLO!`, { identifier: 'success-receive' });
