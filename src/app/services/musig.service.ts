...
export class MusigService {
  ...
  aggregate(storedAccounts, runWithPubkeys = null) {
    ...
    for (let address of addresses) {
      ...
      if (!address.startsWith('oslo_')) {
        throw new Error('Nano addresses must start with oslo_');
      }
      address = address.split('_', 2)[1];
      try {
        const bytes = base32.decode(address);
        if (bytes.length !== 37) {
          throw new Error('Wrong oslo address length');
        }
        ...
        if (!this.util.array.equalArrays(bytes.subarray(32), checksum_)) {
          throw new Error('Invalid oslo address checksum');
        }
        pubkeys.push(pubkey);
      } catch (err_) {
          console.error(err_.toString());
          throw new Error('Invalid oslo address (bad character?)');
      }
    }
    ...
    const fullAddressFinal = 'oslo_' + base32.encode(fullAddress);
