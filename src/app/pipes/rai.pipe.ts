...
export class RaiPipe implements PipeTransform {
  ...
  transform(value: any, args?: any): any {
    ...
    switch (denomination.toLowerCase()) {
      default:
      case 'xrb': return `${(value / this.mrai).toFixed(6)}${!hideText ? ' OSLO' : ''}`;
      case 'mnano':
        ...
        if (hasRawValue) {
          ...
          return `${this.toFixed(newVal, this.precision)}${!hideText ? ' OSLO' : ''}`;
        } else {
          return `${(value / this.mrai).toFixed(6)}${!hideText ? ' OSLO' : ''}`;
        }
