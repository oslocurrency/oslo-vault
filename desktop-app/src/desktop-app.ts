  ...
  constructor() {
    ...
    autoUpdater.on('update-available', (event, releaseNotes, releaseName) => {
      ...
      const dialogOpts = {
        ...
        title: 'New Version',
        message: 'An update for OsloVault is available!',
    ...
    autoUpdater.on('error', message => {
      ...
      mainWindow.setTitle(`OsloVault - ${autoUpdater.currentVersion}`); // reset title
      showUpdateErrors = false; // disable errors
      const dialogOpts = {
        type: 'error',
        buttons: ['OK'],
        title: 'Update Error',
        message: 'Something went wrong while downloading OsloVault.',
...
function createWindow () {
  ...
  mainWindow.webContents.on('did-finish-load', function () {
    mainWindow.setTitle(`OsloVault - ${autoUpdater.currentVersion}`);
  });
...
function sendStatusToWindow(progressObj) {
  ...
  mainWindow.setTitle(`OsloVault - ${autoUpdater.currentVersion} - Downloading Update: ${Math.round(progressObj.percent)} %`);
}
  ...
  const template: any = [
    ...
    },
    {
      ...
      submenu: [
        ...
        {
          label: 'View GitHub',
          click () { loadExternal('https://github.com/oslocurrency/OsloVault'); }
        },
        {
          label: 'Submit a bug report',
          click () { loadExternal('https://github.com/oslocurrency/OsloVault/issues/new'); }
        },
        {
          label: 'Release notes',
          click () { loadExternal('https://github.com/oslocurrency/OsloVault/releases'); }
        },
        {type: 'separator'},
        {
